...
router.post('/get_requested_friends', verify, async (req, res) => {
  ...
  try {
    ...
    let endFor =
      thisUser.friendRequestReceived.length < index + count
        ? thisUser.friendRequestReceived.length
        : index + count
    for (let i = index; i < endFor; i++) {
      ...
      sentUser = await User.findById(sentUserID).select({
        friends: 1,
        friendRequestSent: 1,
        phoneNumber: 1,
        _id: 1,
      ...
      newElement.firstName = sentUser.firstName
      newElement.lastName = sentUser.lastName
      ...
      if (thisUser.friends.length != 0 && sentUser.friends.length != 0) {
        newElement.same_friends = countSameFriend(
          thisUser.friends,
          sentUser.friends,
        )
      }
      newElement.created = validTime.timeToSecond(
        thisUser.friendRequestReceived[i].lastCreated,
      )
...
router.post('/set_request_friend', verify, async (req, res) => {
  ...
  if (id == user_id) {
    ...
    try {
      ...
      let indexExist = thisUser.friends.findIndex((element) =>
        element.friend._id.equals(targetUser._id),
      )
      if (indexExist >= 0)
        return callRes(
          res,
      ...
      let findIndexBlockTarget = targetUser.blockedList.findIndex((e) =>
        e.user._id.equals(thisUser._id),
      )

      // đang bị bên kia block
      if (findIndexBlockTarget >= 0)
        return callRes(
          res,
          responseError.NOT_ACCESS,
          'bị block rồi em ơi ko gửi kb được',
        )
      else {
        // ko bị bên kia block, xóa block bên kia đi (nếu có)
        let findIndexBlockThis = thisUser.blockedList.findIndex((e) =>
          e.user._id.equals(targetUser._id),
        )
        if (findIndexBlockThis >= 0)
          thisUser.blockedList.splice(findIndexBlockThis, 1)
      }
      // add new element to sent request
      let addElement = { _id: targetUser._id }
      let isExisted = thisUser.friendRequestSent.findIndex((element) =>
        element._id.equals(addElement._id),
      )

      if (isExisted < 0) {
        // chưa gửi yêu cầu trước đó
        ...
        // đã gửi yêu cầu trước đó, gửi lại để hủy yêu cầu
...
router.post('/set_block', verify, async (req, res) => {
  ...
  try {
    ...
    return callRes(
      res,
      responseError.PARAMETER_VALUE_IS_INVALID,
      'Cannot find user wanting to block',
    )
  }
  if (targetUser == null) {
    return callRes(
  ...
  if (targetUser.isBlocked) {
    return callRes(
      res,
      responseError.USER_IS_NOT_VALIDATED,
      'User wanted to block has been blocked by server',
    )
  }
  if (!targetUser) {
    return callRes(
      res,
      responseError.USER_IS_NOT_VALIDATED,
      'User wanted to block not existed',
    )
  } else {
    let index = thisUser.blockedList.findIndex((element) =>
      element.user._id.equals(targetUser._id),
    )
    let indexFriend = thisUser.friends.findIndex((element) =>
      element.friend._id.equals(targetUser._id),
    )
    let indexTargetFriend = targetUser.friends.findIndex((element) =>
    ...
    try {
      ...
      if (is_accept == 0) {
        ...
        if (indexExist < 0)
          return callRes(
            res,
            responseError.METHOD_IS_INVALID,
            'không có lời mời',
          )
        ...
        let gg = thisUser.blockedList.findIndex((e) =>
          e.user._id.equals(sentUser._id),
        )
...
router.post('/get_list_blocks', verify, async (req, res) => {
  ...
  let endFor =
    targetUser.blockedList.length < index + count
      ? targetUser.blockedList.length
      : index + count
  for (let i = index; i < endFor; i++) {
...
router.post('/get_user_friends', verify, async (req, res) => {
  ...
  try {
    ...
    await targetUser.populate({ path: 'friends', populate: { path: 'friend' } })
    // .execPopulate()
    ...
    for (let i = index; i < endFor; i++) {
      ...
      if (!thisUser._id.equals(x.friend._id))
        if (thisUser.friends.length > 0 && x.friend.friends.length > 0) {
          friendInfor.same_friends = countSameFriend(
            thisUser.friends,
            x.friend.friends,
          )
        }
    ...
    if (data.friends.length == 0)
      return callRes(res, responseError.NO_DATA_OR_END_OF_LIST_DATA, 'friends')
...
router.post('/get_list_suggested_friends', verify, async (req, res) => {
  try {
    ...
    thisUser = await User.findById(id)
    if (!thisUser)
      return callRes(res, responseError.NO_DATA_OR_END_OF_LIST_DATA, 'thisUser')
        ...
        for (let y of targetUser.friends) {
          if (!y.friend._id.equals(id) && !listID.includes(y.friend._id)) {
            ...
            if (thisUser.friends.length > 0 && y.friend.friends.length > 0) {
              e.same_friends = countSameFriend(
                thisUser.friends,
                y.friend.friends,
              )
            }
